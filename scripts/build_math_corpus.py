...
MATH_SYMBOLS = [
    {
        ...
        'example': 'KE + PE = E_total -> (½mv^2) + (mgh) = constant [all Joules]',
    },
    {
    ...
    {
        'symbol': 'x', 'latex': r'\times', 'name': 'Multiplication / Cross',
        ...
        'real_world': 'Scaling: 3 m x 4 m = 12 m^2 (area). Rate x time = distance. In physics: torque τ = r x F (cross product, units N.m). Power = V x I (Watts).',
        'dimensional_note': 'Unlike addition, multiplication COMBINES units: [m/s] x [s] = [m]. The result has the product of the input units.',
        'context_note': 'In vector calculus: a x b produces a vector perpendicular to both a and b with magnitude |a||b|sin(θ). In set theory: A x B is the Cartesian product (set of ordered pairs).',
        'common_confusion': 'x (cross product) vs . (dot product): cross product gives a vector, dot product gives a scalar.',
    },
    {
        ...
        'real_world': 'Rate calculation: 120 km ÷ 2 h = 60 km/h. Density = mass ÷ volume (kg/m^3). Concentration = moles ÷ liters (mol/L). Efficiency = output ÷ input.',
        'dimensional_note': 'Division divides units: [kg] ÷ [m^3] = [kg/m^3]. This is how derived units are formed.',
    },
    {
        ...
        'real_world': 'E = mc^2 states energy and massxc^2 are the same quantity measured in different ways. An equation is a constraint -- it restricts what values variables can take.',
        ...
        'common_confusion': '= (equals) vs ≡ (identically equal/congruent) vs := (defined as). x^2 − 1 = 0 is an equation (true for specific x). (x+1)(x−1) ≡ x^2 − 1 is an identity (true for all x).',
    },
    {
        'symbol': '!=', 'latex': r'\neq', 'name': 'Not Equal',
        'domains': ['All mathematics'],
    ...
    },
    {
        ...
        'real_world': 'pi ~= 3.14159. g ~= 9.8 m/s^2. For small angles: sin(θ) ~= θ (in radians) when θ < 0.1 rad. Taylor series truncation: e^x ~= 1 + x for small x.',
    ...
    },
    {
        ...
        'real_world': 'F ∝ ma (Newton: force proportional to mass x acceleration). Ohm\'s law: V ∝ I (voltage proportional to current, constant = resistance). Gravitational force ∝ 1/r^2 (inverse square law). When you say "y ∝ x", you mean y = kx for some constant k.',
    ...
    {
        'symbol': '^2  ^3  ⁿ or x^n', 'latex': 'x^n', 'name': 'Exponentiation / Power',
        ...
        'real_world': 'Area = L^2 (square of length). Volume = L^3. Energy in springs: E = ½kx^2. Compound interest: A = P(1+r)^n. Population growth: N(t) = N₀ x 2^(t/T).',
        'dimensional_note': '[x^n] = [x]^n. If x is in meters, x^3 is in cubic meters (m^3).',
        'context_note': 'Negative exponent: x^(−n) = 1/x^n. Fractional exponent: x^(1/n) = ⁿ√x. Zero: x^0 = 1 (for x!=0). In quantum mechanics: |ψ|^2 = probability density (complex squaring).',
    },
    {
        ...
        'real_world': 'RMS voltage: V_rms = √(V₁^2 + V₂^2)/√2. Standard deviation: σ = √(variance). Speed from kinetic energy: v = √(2KE/m). Hypotenuse: c = √(a^2 + b^2).',
        'common_confusion': '√x^2 = |x|, not x (because √ always returns non-negative). For x = −3: √(x^2) = √9 = 3 = |−3|.',
    },
    {
        ...
        'real_world': '|x − target| < ε means x is within ε of the target. |v| = speed (magnitude of velocity vector). |z| for complex z = a+bi: modulus = √(a^2+b^2) = distance from origin.',
        'context_note': 'For matrices: |A| = det(A) = scalar. For vectors: |v| = ‖v‖ = length. For complex: |z| = √(a^2+b^2). For real: |x| = x if x>=0, −x if x<0.',
    },
    {
        'symbol': '!', 'latex': 'n!', 'name': 'Factorial',
        'domains': ['Combinatorics', 'Analysis', 'Number Theory'],
        'meaning': 'n! = n x (n−1) x (n−2) x ⋯ x 2 x 1. Counts the number of ways to arrange n distinct objects.',
        ...
        'context_note': '0! = 1 by convention (empty product). n! grows faster than any exponential: 100! ~= 9.3 x 10¹⁵⁷.',
    },
    {
    ...
    },
    {
        ...
        'real_world': 'Total distance = Σ(speed_i x time_i). Sample mean: x̄ = (1/n)Σxᵢ. Taylor series: sin(x) = Σ(−1)ⁿx^(2n+1)/(2n+1)!. Expected value: E[X] = Σ xᵢ P(xᵢ).',
    ...
    },
    {
        ...
        'meaning': 'Product of a sequence of terms. Πᵢ₌₁ⁿ aᵢ = a₁ x a₂ x ⋯ x aₙ.',
        ...
        'common_confusion': 'Π (capital pi, product notation) vs pi (lowercase pi, circle constant 3.14159...).',
    },
    {
    ...
    },
    {
        ...
        'context_note': 'Notations: f\'(x) (Lagrange), dy/dx (Leibniz), ẋ (Newton dot, for time derivatives), Df (operator). Higher derivatives: f\'\'(x) = d^2y/dx^2.',
        'example': 'If s(t) = ½gt^2 then s\'(t) = gt (velocity increases linearly with time)',
    },
    {
    ...
    },
    {
        ...
        'real_world': 'Total distance = ∫ v(t) dt (sum of all velocityxtime slices). Total charge = ∫ I(t) dt. Probability: P(a<=X<=b) = ∫ₐᵇ f(x)dx. Work = ∫ F.dx. Mass of rod with variable density = ∫ ρ(x) dx.',
        'dimensional_note': '[∫f(x)dx] = [f(x)]x[x]. Integrating force (N) over distance (m) gives energy (N.m = J).',
        'context_note': 'The dx part is not just notation -- it represents an infinitesimal width. ∫ₐᵇ f(x)dx = lim_{n->inf} Σ f(xᵢ)Δx. The elongated S (∫) is from Latin "summa" (sum).',
    ...
    },
    {
        ...
        'real_world': '∇T = temperature gradient (points toward hottest direction, tells heat flow direction). ∇.E = ρ/ε₀ (Gauss\'s law: divergence of E-field = charge density). ∇xB = μ₀J (Ampere\'s law: curl of B-field = current density). ∇^2 = Laplacian (used in wave equations, heat equation).',
        'context_note': 'In 3D Cartesian: ∇ = (∂/∂x, ∂/∂y, ∂/∂z). Operations: ∇f = gradient (scalar->vector), ∇.F = divergence (vector->scalar), ∇xF = curl (vector->vector), ∇^2f = Laplacian (scalar->scalar).',
    },
    {
    ...
    {
        'symbol': 'inf', 'latex': r'\infty', 'name': 'Infinity',
        ...
        'real_world': 'lim_{n->inf} (1 + 1/n)^n = e. ∫₀^inf e^(−x^2) dx = √pi/2 (Gaussian integral, fundamental in probability). Infinite series: Σ_{n=1}^inf 1/n^2 = pi^2/6 (Basel problem).',
        'context_note': 'Different sizes: ℕ, ℤ, ℚ all have countable infinity (ℵ₀). ℝ has uncountable infinity (ℵ₁ = 2^ℵ₀). inf + 1 = inf in the extended real line but inf − inf is indeterminate.',
    ...
    },
    {
        ...
        'real_world': 'Continuous growth/decay: N(t) = N₀ eᵏᵗ. Radioactive decay: N(t) = N₀ e^(−λt). RC circuits: V(t) = V₀ e^(−t/RC). Normal distribution: e^(−x^2/2). Fourier analysis: e^(iωt) = cos(ωt) + i.sin(ωt).',
        'context_note': 'e = lim_{n->inf}(1 + 1/n)^n = Σ_{n=0}^inf 1/n!. In complex analysis: e^(ipi) + 1 = 0 (Euler\'s identity, linking e, i, pi, 1, 0).',
    },
    {
        'symbol': 'pi', 'latex': r'\pi', 'name': 'Pi',
        'domains': ['Geometry', 'Trigonometry', 'Analysis', 'Physics'],
        'meaning': 'Ratio of circle\'s circumference to its diameter. pi ~= 3.14159265...',
        'how_to_read': '"pi"',
        'real_world': 'Circumference = 2pir. Area = pir^2. Volume of sphere = (4/3)pir^3. Period of pendulum: T = 2pi√(L/g). In signal processing: angular frequency ω = 2pif. Heisenberg uncertainty: ΔxΔp >= ħ/2 where ħ = h/(2pi).',
        'context_note': 'pi appears in many non-circular contexts because it is fundamentally tied to the geometry of the Euclidean plane, Fourier analysis, and eigenvalues of differential operators.',
    },
    {
        ...
        'context_note': 'i^2 = −1, i^3 = −i, i⁴ = 1. |a + bi| = √(a^2 + b^2). Argument: arg(a+bi) = arctan(b/a). Polar form: re^(iθ) = r(cos θ + i sin θ).',
    },
    {
    ...
    },
    {
        ...
        'real_world': 'Wave: y(x,t) = A.sin(kx − ωt + φ). Projectile: x = v₀.cos(θ).t, y = v₀.sin(θ).t − ½gt^2. AC voltage: V(t) = V₀.cos(ωt). Power in 3-phase: P = √3.V.I.cos(φ).',
        'dimensional_note': 'The argument of trig functions must be dimensionless (radians are dimensionless). sin(30deg) requires conversion: sin(pi/6). Result is always between −1 and 1 (for sin and cos).',
    },
    {
        ...
        'context_note': 'Output range: arcsin -> [−pi/2, pi/2], arccos -> [0, pi], arctan -> (−pi/2, pi/2). For full-quadrant angles use atan2(y, x).',
    },
    {
    ...
    },
    {
        ...
        'real_world': 'A ∩ B = ∅ means events A and B are mutually exclusive (cannot both occur). Solution set of x^2 = −1 over ℝ is ∅.',
    },
    {
        ...
        'real_world': '∀ x ∈ ℝ: x^2 >= 0 (every real number squared is non-negative). ∀ ε > 0, ∃ δ > 0 such that... (formal limit definition). Universal laws in physics are written as ∀-statements.',
    ...
    },
    {
        ...
        'real_world': '∃ x ∈ ℝ: x^2 = 2 (√2 is a real number). ∄ x ∈ ℚ: x^2 = 2 (√2 is irrational). In optimization: prove ∃ a minimum before trying to find it.',
    ...
    },
    {
        ...
        'how_to_read': '"mu"',
        'real_world': 'μ = 5.2 kg -> mean mass is 5.2 kg. 5 μm = 5 micrometers (wavelength of infrared). μₖ = 0.3 -> kinetic friction coefficient. μ₀ = 4pix10⁻⁷ H/m (permeability of free space in Ampere\'s law).',
    },
    {
        ...
        'how_to_read': '"sigma"',
        'real_world': 'σ = 2.5 kg -> data spreads ±2.5 kg from mean. 68% of data within 1σ of mean (normal distribution). σ = 50 MPa -> material under 50 megapascal stress. σ = 5.8x10⁷ S/m -> electrical conductivity of copper.',
    ...
    },
    {
        ...
        'how_to_read': '"theta"',
        'real_world': 'θ = 45deg (or pi/4 radians) in projectile launch. θ in polar coordinates (r, θ). Phase angle in AC circuits. In statistics: generic parameter to be estimated.',
    },
    {
    ...
    },
    {
        ...
        'meaning': 'Angular frequency ω = 2pif (radians per second). Angular velocity of rotation.',
        'how_to_read': '"omega"',
        'real_world': 'ω = 2pi x 60 Hz = 376.99 rad/s for 60 Hz AC. Period T = 2pi/ω. In LC circuit: resonant frequency ω₀ = 1/√(LC). Capital Ω = ohms (resistance unit) in electronics.',
    },
    {
        'symbol': 'ρ (rho)', 'latex': r'\rho', 'name': 'Rho -- Density / Resistivity / Radius',
        'domains': ['Physics', 'Chemistry', 'Engineering', 'Fluid Mechanics'],
        'meaning': 'Mass density (kg/m^3), charge density, resistivity of material, radial coordinate in cylindrical coordinates.',
        'how_to_read': '"rho"',
        'real_world': 'ρ_water = 1000 kg/m^3. ρ_copper = 1.72x10⁻⁸ Ω.m (resistivity). In fluid mechanics: ρ appears in Navier-Stokes equations. Pressure: P = ρgh (hydrostatic).',
    },
    {
        ...
        'how_to_read': '"tau"',
        'real_world': 'RC circuit: τ = RC [seconds]. After time τ, signal decays to 1/e ~= 37% of original. Torque: τ = r x F [N.m]. After 5τ, a system is considered settled (~= 99.3% of final value).',
    },
    {
        'symbol': 'alpha beta γ', 'latex': r'\alpha, \beta, \gamma', 'name': 'Greek Letters -- General Parameters',
        'domains': ['Physics', 'Statistics', 'Mathematics generally'],
        'meaning': 'alpha, beta, γ are extremely common general-purpose parameter symbols. Meaning entirely context-dependent.',
        'how_to_read': '"alpha", "beta", "gamma"',
        'real_world': 'alpha: significance level in statistics (alpha=0.05), angular acceleration, fine-structure constant. beta: regression coefficient, beta decay, velocity as fraction of c. γ: Lorentz factor γ=1/√(1−v^2/c^2), heat capacity ratio Cp/Cv.',
    },
    {
        ...
        'real_world': '(AB)^T = B^T A^T. For rotation matrix R: R^T = R^(−1) (orthogonal matrix property). In least squares: optimal fit beta = (X^T X)^(−1) X^T y.',
    },
    {
    ...
    },
    {
        ...
        'real_world': '‖v‖ = √(v₁^2 + v₂^2 + ⋯ + vₙ^2) (Euclidean norm). Unit vector: v̂ = v/‖v‖. Speed = ‖velocity vector‖. Distance = ‖position₂ − position₁‖.',
    },
    {
    ...
    },
    {
        ...
        'real_world': 'Bubble sort: O(n^2) operations. Binary search: O(log n). Describes how algorithm time/space grows with input size. Physics: Taylor series remainder term O(x^3) means error is bounded by Cx^3 for small x.',
    ...
    },
    {
        ...
        'real_world': '17 ≡ 2 (mod 5) because 17 = 3x5 + 2 and 2 = 0x5 + 2. Clock arithmetic: 14:00 ≡ 2 (mod 12). Cryptography (RSA): based on modular exponentiation.',
    },
    {
    ...
    },
    {
        ...
        'meaning': 'E[X]: average value of random variable X over many trials. Var(X) = E[(X−μ)^2]: average squared deviation from mean.',
        'how_to_read': '"expected value of X" | "variance of X"',
        'real_world': 'E[X] = μ = mean. Var(X) = σ^2. Insurance: E[payout] determines fair premium. E[X+Y] = E[X] + E[Y] always. Var(X+Y) = Var(X) + Var(Y) only if X,Y independent.',
    },
    {
        ...
        'real_world': 'X ~ N(μ, σ^2): X is normally distributed. X ~ Poisson(λ): X counts rare events per interval. X ~ Bernoulli(p): X is 1 with probability p, 0 otherwise.',
    },
    {
        'symbol': 'ħ (h-bar)', 'latex': r'\hbar', 'name': 'Reduced Planck Constant',
        'domains': ['Quantum Mechanics', 'Physics'],
        'meaning': 'ħ = h/(2pi) ~= 1.055x10⁻^3⁴ J.s. Fundamental quantum of angular momentum.',
        'how_to_read': '"h-bar"',
        'real_world': 'Heisenberg uncertainty: ΔxΔp >= ħ/2. Energy of photon: E = ħω. Schrodinger equation: iħ ∂ψ/∂t = Ĥψ. Spin angular momentum: s = ½ħ for electrons.',
    },
    {
        ...
        'real_world': '⟨ψ|Ĥ|ψ⟩ = expected energy. ⟨ψ|x̂|ψ⟩ = expected position. |⟨φ|ψ⟩|^2 = probability of measuring state φ when system is in state ψ.',
    },
]
...
error-checking tools in science.
  E = ½mv^2 + mgh
  Check: ½mv^2 -> kg x (m/s)^2 = kg.m^2/s^2 = J [ok]
         mgh  -> kg x m/s^2 x m = kg.m^2/s^2 = J [ok]
  Both terms are in Joules -- dimensionally consistent. [ok]
...
VARIABLE NAMING CONVENTIONS (informal but widely followed)
  ...
  alpha, beta, γ       -> angles or generic parameters
...
READING A SUM OR SERIES
  ...
  The index n is a dummy variable -- it's just counting through the terms:
    ...
    n=1: x¹/1! = x
    n=2: x^2/2! = x^2/2
    n=3: x^3/3! = x^3/6   ... and so on forever.
...
The SAME symbol can mean different things in different fields:
  ...
  x    -> multiplication vs cross product vs Cartesian product
...
DIMENSIONAL ANALYSIS AS PROBLEM-SOLVING
  To find the formula for the period of a pendulum T, we know:
    T depends on: length L [m], gravity g [m/s^2], mass m [kg]
    T has units: [s]

    Try T = k x Lᵃ x gᵇ x mᶜ
    [s] = [m]ᵃ x [m/s^2]ᵇ x [kg]ᶜ
    [s¹] = [m^(a+b)] x [s^(−2b)] x [kg^c]

    Matching: −2b = 1 -> b = −½, a+b = 0 -> a = ½, c = 0.
    Therefore: T = k√(L/g)  (k = 2pi from full derivation)
...
MATH_WIKI_TOPICS = {
    'Foundations & Logic': [
        ...
        'Model theory', 'Proof theory', 'Godel\'s incompleteness theorems',
    ...
    'Differential Equations': [
        ...
        'Laplace\'s equation', 'Schrodinger equation',
    ],
    'Topology': [
        ...
        'Euler characteristic', 'Mobius strip', 'Klein bottle', 'Torus',
    ],
...
def train_stage34(node: str, data_dir: str, args):
    ...
    for sym in MATH_SYMBOLS:
        ...
        if _train(node, text, session):
            ...
            print(f'  [ok] Symbol: {sym["symbol"]} ({sym["name"]})')
        time.sleep(0.05)
    ...
    if guide_key not in trained_wiki:
        ...
        if _train(node, READING_MATH_GUIDE, session):
            ...
            print('  [ok] Reading mathematics guide trained.')
        ...
        for topic in topics:
            ...
            if text:
                ...
                if _train(node, full, session):
                    ...
                    print(f'  [{done}/{total_topics}] [ok] {branch}: {topic}')
            time.sleep(0.15)
        _save_checkpoint(ckpt_wiki, trained_wiki)
