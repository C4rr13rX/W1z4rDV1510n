...
The bovine ear (pinna) acts as a directional sound receptor and as a thermoregulatory organ; the auricular vasculature radiates heat. Normal internal ear canal temperature (~38.5degC) is used for remote temperature monitoring in electronic ear tags.''',
...
The ovaries are bean-shaped, approximately 3x2x1.5 cm in the cycling cow. Follicular development follows a wave pattern (2-3 waves per 21-day oestrous cycle). The dominant follicle of the first and second waves is suppressed by progesterone from the corpus luteum unless the animal is in oestrus.

In the male (bull), the scrotum contains the testes in a thermoregulated environment 3-5degC below core body temperature. Each testis is approximately 12 x 7 x 6 cm and weighs 300-400 g. Spermatogenesis takes ~54-60 days in cattle. The penis is fibroelastic (sigmoid flexure type, unlike the vascular type in primates), maintained in the prepuce by retractor penis muscles.''',
...
Milk is synthesised in MECs by: fatty acid synthesis (de novo from acetate/beta-hydroxybutyrate), triglyceride assembly in smooth ER, protein synthesis (casein, whey proteins) in rough ER/Golgi, and lactose synthesis in Golgi (lactose synthetase). MECs express numerous nutrient transporters (GLUT1, MCT1, amino acid transporters) for substrate uptake.
...
The teat consists of: teat skin (epidermis), teat wall (connective tissue + smooth muscle), teat cistern (gland cistern + teat cistern separated by Furstenberg's rosette), and teat canal (streak canal, ~10 mm long, sealed by Furstenberg's rosette and keratin plug between milkings). The teat canal is the primary defence against mastitis.''',
...
Bovine bone mineral density (BMD) is approximately 1.5-1.7 g/cm^3 for cortical bone. Calcium content is ~34% by weight. Phosphorus content ~15% by weight. The Ca:P ratio of 2.2:1 in hydroxyapatite matches the dietary requirement ratio for cattle (1.5-2:1).
...
Bone density in mature Holstein cows measured by dual-energy X-ray absorptiometry (DXA): L2-L4 BMD ~= 1.1-1.4 g/cm^2, similar to humans. Transition cow mobilisation of bone calcium begins in early lactation when demand exceeds dietary supply.''',
...
Omasum ("Bible tripes"): 100-200 muscular laminae (leaves) lined by stratified squamous epithelium with short papillae. Function: water absorption and fine-particle trapping. The laminae expand surface area to ~10 m^2 -- more than 10x the omasal luminal surface.
...
Thyroid gland: two lobes connected by an isthmus at the laryngeal-tracheal junction. Follicular cells synthesise T4 (thyroxine) and T3 (triiodothyronine) from tyrosine residues within thyroglobulin. Bovine T3 is biologically 3-4x more potent than T4 (T3 is the active form after peripheral deiodination). C-cells (parafollicular cells) secrete calcitonin -- important in calcium homeostasis in lactating cows (calcitonin inhibits osteoclast activity and reduces Ca2+ resorption from bone).
...
Pancreas: both exocrine (acinar cells secreting digestive enzymes) and endocrine (islets of Langerhans: alpha-cells/glucagon, beta-cells/insulin, δ-cells/somatostatin). Bovine insulin was the first protein hormone to be sequenced (Sanger, 1953); bovine insulin differs from human insulin at 3 amino acid positions and was used therapeutically in humans before recombinant human insulin.''',
...
Fetal sex differentiation: the gonadal ridge forms day 30. Testes in males secrete AMH (anti-Mullerian hormone, causing regression of Mullerian ducts) and testosterone (promoting Wolffian duct development: epididymis, vas deferens, seminal vesicles). In females, ovaries passively develop; oocytes enter meiotic prophase I arrest (dictyotene) by day 100.
...
The trot is rare in cattle; they transition to a gallop (3-beat asymmetric) or a run. Cattle reach a maximum speed of approximately 8-9 m/s (29-32 km/h). The trot-gallop transition occurs at a Froude number of approximately 0.5 (Fr = v^2/gL, where L = leg length). Cattle have a shorter relative leg length than horses, shifting their preferred trot-gallop transition speed to lower velocities.

Ground reaction forces in cattle: peak vertical GRF approximately 1.5-2.0x body weight during walking. The hindlimb peak GRF occurs slightly later in the stance phase than the forelimb. Hoof-ground contact time: 350-550 ms at a walk. Digital pulse (palpable increase in digital artery pulse amplitude) is detectable in laminitic cows due to increased blood flow and hoof capsule temperature.

Kinematic hoof analysis: the hoof angle (angle of the dorsal hoof wall to the ground) should be 45-55deg in the forefeet and 50-60deg in the hindlimb lateral claws. Overgrowth of the toe leads to a broken-forward hoof angle; excessive heel growth causes a broken-backward angle. Both alter biomechanics and predispose to lameness. Cow comfort scoring and locomotion scoring (1-5 scale) assess gait symmetry and back curvature.''',
...
#   * Each level x modality = 1 synthetic cross-section image + 1 text document
#
...
def generate_mri_ct_data(out_dir: Path, n_noise_levels: int = 8) -> list[dict]:
    ...
    Produces 3 modalities x 16 anatomical levels x n_noise_levels images,
    ...
    print(f'  Generated {len(items)} MRI/CT items '
          f'({len(MRI_CT_ANATOMY_LEVELS)} levels x {len(MODALITIES)} modalities x '
...
BOVINE_QA_PAIRS: list[tuple[str, str]] = [
    ...
    ("What hormones regulate the bovine estrous cycle?",
     "GnRH (from hypothalamus) -> LH and FSH (from anterior pituitary) -> follicle development and estradiol production (from ovary). Estradiol triggers LH surge -> ovulation. Corpus luteum produces progesterone. Luteolysis via uterine PGF2alpha (prostaglandin F2-alpha) resets the cycle."),
    ...
    ("What is the corpus luteum in cattle?",
     "The corpus luteum (CL) is the progesterone-secreting endocrine structure that forms from the ruptured follicle after ovulation. It maintains pregnancy by suppressing GnRH/LH release. Luteolysis (CL regression) is triggered by uterine PGF2alpha on days 16-18 if the cow is not pregnant."),
    ...
    ("How does bovine vision differ from human vision?",
     "Cattle are dichromats (two types of cone photoreceptors), perceiving primarily blue and yellow-green wavelengths but not red. Their horizontal pupil provides approximately 330deg panoramic vision with a binocular field of only 25-30deg ahead, favoring predator detection over depth perception. The tapetum lucidum enables effective dim-light vision."),
    ("What is the bovine thermoregulatory neutral zone?",
     "The thermal neutral zone (TNZ) for cattle is approximately 5-25degC (41-77degF). Below 5degC, metabolic rate increases to maintain core temperature. Above 25degC, heat dissipation via panting, sweating, and vasodilation increases. Heat stress index (THI >72) significantly reduces feed intake and milk production."),
    ...
    ("What is the bovine normal body temperature?",
     "The normal bovine rectal temperature is 38.0-39.5degC (100.4-103.1degF). Values above 39.5degC indicate fever; above 41degC indicates severe hyperthermia. Calves have slightly higher normal temperatures (38.5-40degC). Evening temperatures are typically 0.5degC higher than morning temperatures."),
    ...
    ("What are the histological layers of the bovine small intestine?",
     "The bovine small intestinal wall has four layers: mucosa (columnar epithelium with villi and crypts of Lieberkuhn, goblet cells, enterocytes), submucosa (Brunner's glands in duodenum, blood and lymph vessels), muscularis externa (inner circular and outer longitudinal smooth muscle), and serosa (visceral peritoneum)."),
...
# 65+ Sketchfab categories x up to 150 paginated models = ~10K training items.
...
def build_mesh_training(out_dir: Path, node: str,
    ...
    Four sub-stages:
      ...
      B. Sketchfab: paginated search across 65+ categories x 100 models = ~6K thumbnails.
