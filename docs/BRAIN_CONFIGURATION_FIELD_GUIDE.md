...
| 2026-07-15 | A failed 16,384-row canary should quarantine only the range after the latest passing canary, while the immutable full-gate snapshot remains the rollback authority. Replay the earlier green prefix after rollback and focus micro-brain bisection on `suspect_start_row..suspect_end_row`; do not repeatedly diagnose the entire 131,072-row admission block. | Contract-tested triage rule | `scripts/programming_curriculum_supervisor.py` and `tests/test_programming_runtime_contract.py` |
