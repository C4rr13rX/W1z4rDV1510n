...
For very large corpora, a “block” must be bounded rather than an entire corpus phase. The current programming curriculum snapshots every 4,096 durable rows and, after two clean calibration cycles, gates guarded 16,384-row blocks. Each guarded block runs distributed corpus recall (including the first and newest trained rows), complete foundational retention, and the strict enterprise battery before the next block is permitted. Separating the 4,096-row crash-recovery cadence from the 16,384-row evaluation cadence bounds both WAL replay and benchmark overhead. A checkpoint without a retention gate proves durability, not non-interference.
