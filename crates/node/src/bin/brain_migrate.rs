...
fn main() -> Result<()> {
    ...
    let required_bytes = checkpoint_bytes.saturating_add(reserve_bytes);
    anyhow::ensure!(
        available_bytes >= required_bytes,
        "refusing migration: {:.2} GiB available, but the {:.2} GiB checkpoint plus the 8 GiB host reserve requires at least {:.2} GiB; close applications or use the forthcoming streaming converter",
